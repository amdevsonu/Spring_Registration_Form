...
public class StudentController {
	...
	private void loadFormData(Model model) { //Common or boilerplate method to call or use to load the form
	...
	public String handleSubmit(Student s, Model model) { //This method is used to handle the PostRequest
		...
		entity.setTiming(Arrays.toString(s.getTiming())); //manually set the time and convert from String of array to string format
		repo.save(entity); //save the data 
		return "index";
	}
	
	@GetMapping("/viewStudents")
	public String getStudentData(Model model) { //this method is used to show saved student data in anathor page
		
		List<StudentEntity> studentList = repo.findAll(); //Get all the list of students
		model.addAttribute("students", studentList); //to send the student data from controller to UI
		return "data";
	}
}
